...
class ScheduleFinder {
  ...
  async _checkPeriodAvailable(scheduleHandle, bookingParams) {
    const now = new Date()
    const maxBookingTimestamp = new Date(now.getFullYear(), now.getMonth(), now.getDate() + bookingParams.maxBookingDelay);
    ...
    if (scheduleHandle){
      ...
      isNextSchdeduleOverlap = await scheduleHandle.evaluate(this.findIfNextBookedScheduleOverlapsPickedSchedule, 
        ...
        bookingParams,
        maxBookingTimestamp)
        console.log('isNextScheduleOverlap', isNextSchdeduleOverlap)
